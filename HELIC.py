...
#        pickle.dump(HCSpace, open(dataName+'HCSpace.data', 'wb'))
    ...
    print(len(W))
    embedData = fc.generate_kernelVector(data, KernelSpace, W, reduce = True)
...
#        dataName = 'hayesroth'
        ...
        print(dataName,'is not in the data lib.')
        continue
    ###for python data
    ...
    kernelPara = {'Guassian': [2**int(i) for i in para], 'Polynominal': [(0,1), (0,2), (0,3)]}
    #skf = StratifiedKFold(label[:,0], n_folds = splitNum)
    #for train,test in skf:
    try:
        for p_index in range(20):
            X_train, X_test, y_train, y_test = data[trnIndex[p_index,:]-1], data[tstIndex[p_index,:]-1], label[trnIndex[p_index,:]-1], label[tstIndex[p_index,:]-1]
        ...
        #    resKNN[index] = neigh.score(embed_test, y_test[:,0])
            ...
            if os.path.isfile('log_HELIC_results_detail_2.csv'):
                csvFile = open("log_HELIC_results_detail_2.csv", "a+")
                writer = csv.writer(csvFile)
                writer.writerow(log)
                csvFile.close()
            else:
                csvFile = open("log_HELIC_results_detail_2.csv", "w")
                writer = csv.writer(csvFile)
                writer.writerow(log)
                csvFile.close() 
            index += 1
        print('the KNN accuracy on dataset',dataName,'is:', np.mean(resKNN)*100,'±',np.std(resKNN)*100)
        log = [dataName, str(np.mean(resKNN)*100) + '$\pm$' + str(np.std(resKNN)*100)]
        if os.path.isfile('log_HELIC_results_ra.csv'):
            csvFile = open("log_HELIC_results_ra.csv", "a+")
            writer = csv.writer(csvFile)
            writer.writerow(log)
            csvFile.close()
        else:
            csvFile = open("log_HELIC_results_ra.csv", "w")
            writer = csv.writer(csvFile)
            writer.writerow(log)
            csvFile.close()  
    except:
        print('something error on dataset:',dataName)
        continue
